...
To produce results as mentioned in the paper please run following command. You will need following torch packages: `nn`, `nngraph`, `dp`, `dpnn`, `optim` and `cunn` & `cutorch` if using cuda ```--useCuda```.
```
